...
internal data class SafariCritter(
    ...
    val highlightEntities: List<Entity>,
)
...
internal object SafariCritterDetector {
    ...
    private fun NamedCritter.toCritter(
        ...
        val modelEntities = matchedEntities.filter { entity -> entity is Display || entity is ArmorStand && !entity.isMarker }
        return SafariCritter(
            name = kind.name,
            rarity = kind.rarity,
            nameplate = nameplate,
            captureEntity = captureEntity,
            highlightEntities = modelEntities.ifEmpty { listOfNotNull(captureEntity) },
        )
    }
    ...
    private val CRITTER_KINDS = buildList {
        addAll(critters(SkyBlockRarity.COMMON, "Cavernfish", "Flitter", "Shyworm", "Foxtrot", "Strongarm", "Tepid"))
        addAll(
            critters(
